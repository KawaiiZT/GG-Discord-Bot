...
class Todo(commands.Cog):
    ...
    async def add_task(self, interaction, task: str):
        if interaction.author.id not in tasks:
            tasks[interaction.author.id] = []
        tasks[interaction.author.id].append(task)
    ...
    async def view_tasks(self, interaction):
        user_tasks = tasks.get(interaction.author.id, [])
        if not user_tasks:
    ...
    async def clear_tasks(self, interaction):
        if interaction.author.id in tasks:
            tasks[interaction.author.id] = []
